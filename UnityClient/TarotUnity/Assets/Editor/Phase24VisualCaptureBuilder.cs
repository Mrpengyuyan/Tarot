...
{
    /// <summary>
    /// Phase 24 visual capture. Mirrors the Phase 22 capture (edit-mode
    /// camera.Render of the three UI scenes, with framing cards placed in the
    /// reading room) but writes to its own folder so the bundled-font result can
    /// be compared against the earlier system-font screenshots.
    /// </summary>
    public static class Phase24VisualCaptureBuilder
    {
        public const string ReviewFolder = "Docs/VisualReview/Phase24";
        ...
        [MenuItem("Tools/Tarot Unity/Run Phase 24 Visual Capture")]
        public static void Run()
        {
            if (EditorApplication.isPlaying)
            {
                EditorApplication.isPlaying = false;
                Debug.LogWarning("Exited Play Mode. Run Phase 24 Visual Capture again after Unity returns to Edit Mode.");
                return;
            }
            ...
            Debug.Log("Tarot Unity Phase 24 visual capture complete.");
        }
        ...
        private static List<GameObject> PlaceFramingCards()
        {
            ...
            if (prefab == null)
            {
                Debug.LogWarning($"Phase 24 capture could not load card prefab at {CardPrefabPath}.");
                return proxies;
            }
            ...
            foreach (var slotName in FramingSlotNames)
            {
                ...
                proxy.name = $"Phase24_FramingProxy_{slotName}";
                proxies.Add(proxy);
            }
        ...
        private static void WriteManifest()
        {
            ...
            builder.AppendLine("  ]");
            builder.AppendLine("}");
            File.WriteAllText(Path.Combine(ReviewFolder, "phase24_visual_capture_manifest.json"), builder.ToString());
        }
