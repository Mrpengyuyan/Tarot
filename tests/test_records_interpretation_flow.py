...
def test_draw_cards_with_seed_is_reproducible(client, seeded_spread_and_cards):
    ...
    prediction_id_1 = _create_prediction(client, spread_id)
    prediction_id_2 = _create_prediction(client, spread_id)

    draw_1 = client.post(f"/api/v1/records/{prediction_id_1}/draw?seed=20260316")
    draw_2 = client.post(f"/api/v1/records/{prediction_id_2}/draw?seed=20260316")
    assert draw_1.status_code == 200
    assert draw_2.status_code == 200

    cards_1 = client.get(f"/api/v1/records/{prediction_id_1}/cards").json()
    cards_2 = client.get(f"/api/v1/records/{prediction_id_2}/cards").json()
    assert [c["tarot_card_id"] for c in cards_1] == [c["tarot_card_id"] for c in cards_2]
    assert [c["is_reversed"] for c in cards_1] == [c["is_reversed"] for c in cards_2]
