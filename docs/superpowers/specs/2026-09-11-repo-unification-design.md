...
spec 编写期间已经在 `f19c7cd` 的 80 个文件上做过一次：对照命中 6 个文件，真实密钥模式 0 命中。*（更正：本节最初写的是 3 个文件，原因是当时的统计命令末尾带着 `| head -3`，输出被截断。执行 Task 5 时发现，不截断重新统计后确认为 6 个。）*`.env.example` 里的 `SECRET_KEY` 是 10 个小写单词用连字符连起来的短语，看形态是占位符。
